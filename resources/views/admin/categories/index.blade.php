...
@section('title', 'Manage Categories - ZestShop')
...
@section('content')
    ...
    <div class="card-premium">
        ...
        <div class="card-premium-header flex-column flex-sm-row gap-3">
            <div>
                <h2 class="card-premium-title">All Categories</h2>
                <p class="text-muted mb-0" style="font-size: 12px; margin-top: 2px;">{{ $categories->total() }} categories defined</p>
            </div>
            
            <div class="d-flex flex-wrap gap-2 w-100 w-sm-auto justify-content-sm-end">
                <form action="{{ url('/admin/categories') }}" method="GET" class="d-flex gap-2 w-100 w-sm-auto">
                    <div class="position-relative w-100 w-sm-auto" style="min-width: 200px;">
                        <input type="text" name="search" value="{{ request('search') }}" class="form-control rounded-pill px-3 py-1.5" placeholder="Search categories..." style="font-size: 13.5px; border-color: var(--border-color); padding-left: 35px !important;">
                    ...
                    @if(request()->anyFilled(['search', 'status']))
                        <a href="{{ url('/admin/categories') }}" class="btn btn-outline-secondary rounded-pill px-3 d-flex align-items-center" title="Clear Filters" style="font-size: 13px;">
            ...
            <table class="table table-premium">
                ...
                <tbody>
                    @forelse($categories as $category)
                        <tr>
                            <td class="text-muted" style="font-size: 12.5px;">#CAT-{{ sprintf('%03d', $category->id) }}</td>
                            <td>
                                @if($category->image)
                            ...
                            <td class="text-end">
                                <div class="d-inline-flex gap-1">
                                    <a href="{{ url('/admin/categories/' . $category->id . '/edit') }}" class="btn btn-sm btn-outline-secondary border-0 p-2 rounded-circle" style="color: #64748B;" title="Edit Category">
                                        <i class="ti ti-pencil fs-5"></i>
                                    </a>
                                    
                                    <form action="{{ url('/admin/categories/' . $category->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this category? This action cannot be undone.');" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-outline-danger border-0 p-2 rounded-circle" style="color: #EF4444;" title="Delete Category">
